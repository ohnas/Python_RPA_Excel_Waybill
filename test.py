...
db_multi_drop_duplicates_df = db_multi_df.drop_duplicates(keep="first", subset=["id"])
...
select_df = row_df[["주문번호", "수취인명", "옵션정보", "수량", "수취인연락처1", "수취인연락처2", "배송지", "배송메세지"]]
...
for df_row in drop_duplicated_df.to_dict("records"):
    for db_row in db_single_df.to_dict("records"):
        if df_row["품목명"] == db_row["품목명"] and df_row["내품수량"] == db_row["내품수량"]:
            ...
            drop_duplicated_df.at[df_row["고객주문번호"], "보내는분주소(전체, 분할)"] = db_row["보내는분주소(전체, 분할)"]
            drop_duplicated_df.at[df_row["고객주문번호"], "보내는분전화번호"] = db_row["보내는분전화번호"]
            drop_duplicated_df.at[df_row["고객주문번호"], "보내는분기타연락처"] = db_row["보내는분기타연락처"]
        else:
            continue
